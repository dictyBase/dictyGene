...
// Otherwise, it wont show up.   
   ...
   function validate_form(form_name)
   {
	...
	function validate_order(myform) {
    		var names = ['organization',  'address1',  'city',  'zip',  'country',  'phone',  'email',  'first_name' , 'last_name'];
    		var empty = new RegExp("^\\s+$");
    		
    		for (i in names) {
    			var element = myform.elements[names[i]];
    			if (!element.value || empty.test(element.value)) {
						alert('Please enter a valid value for the following form fields\n' + names[i]);
						return false;	
    			} 
    		}
