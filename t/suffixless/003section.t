...
$tx = Mojo::Transaction->new_get("/gene/$gene_id/gene/genomic_info.json");
$client->process_app( 'DictyREST', $tx );
is( $tx->res->code, 200, 'is a successful response for info section' );
like( $tx->res->headers->content_type,
...
#request for product section
$tx = Mojo::Transaction->new_get("/gene/$gene_id/gene/product.json");
$client->process_app( 'DictyREST', $tx );
is( $tx->res->code, 200, 'is a successful response for info section' );
like( $tx->res->headers->content_type,
    qr/json/, 'is a json content for product' );
...
$tx = Mojo::Transaction->new_get(
    "/gene/$gene_id/protein/$transcript_id/sequence");
$client->process_app( 'DictyREST', $tx );
is( $tx->res->code, 200,
...
$tx = Mojo::Transaction->new_get(
    "/gene/$gene_id/protein/$transcript_id/sequence.json");
$client->process_app( 'DictyREST', $tx );
is( $tx->res->code, 200,
...
$tx = Mojo::Transaction->new_get("/gene/$gene_id/feature");
$client->process_app( 'DictyREST', $tx );
...
$tx = Mojo::Transaction->new_get("/gene/$gene_id/feature/$transcript_id");
$client->process_app( 'DictyREST', $tx );
is( $tx->res->code, 200, 'is a successful response for feature section' );
like(
    $tx->res->body,
